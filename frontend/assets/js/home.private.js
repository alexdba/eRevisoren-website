...
$.vegas('slideshow', {
  delay: 6000,
  backgrounds: [
    {src: config.baseUrl + '/assets/images/home/attention-row.jpg', fade: 1000},
  ]
});
*/
...
$(document).ready(function () {
  $('.main-nav-list').onePageNav({
    ...
    filter: ':not(.external)',
        ...
        //move 50px before selected section
        $('body').scrollTo('-=50px',800);
    }
  });
  ...
  $(window).scroll(function (event) {
    ...
    if(y > 0){
        if($('#main-nav').hasClass('scroll0')){
            $('#main-nav').addClass('scroll1').removeClass('scroll0');
        }
    }
    else{
        if($('#main-nav').hasClass('scroll1')){
            $('#main-nav').addClass('scroll0').removeClass('scroll1');
        }
    }
  });
  ...
  $('.first-menu-a').addClass("current");
    ...
    if(y > 0){
        $('#main-nav').removeClass('scroll0').addClass('scroll1');
    }
    else
        $('#main-nav').removeClass('scroll1').addClass('scroll0');
});
